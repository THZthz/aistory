...
def _get_wiki(cursor, wiki_name):
    cursor.execute('''SELECT description FROMwikis_table WHERE name = %s''', (wiki_name,))
...
def _get_useful_wikis(cursor, embedding, description: str = None, desc_embedding: torch.Tensor = None):
    ...
    cursor.execute(
        '''SELECT name, description, (desc_embedding <#> %s::vector) * -1 AS similarity FROMwikis_table ORDER BY similarity DESC LIMIT 4''',
