...
class Qualities(Enum):
    IronWood = Common = 0  # IronWood and Common will return 'ironwood' if __str__ is called.
    SilverVein = Uncommon = 1
    GoldLeaf = Rare = 2
    DuskForged = Exquisite = 3
    DawnBorn = Legendary = 4
    ...
    def _update_from_database(self):
        def _callback1(cursor):
            cursor.execute('''SELECT type_name, category_id, type_quality, type_description, type_attributes 
                FROM "ItemTypesTable" WHERE type_id = %s''', (self.id,))
            res = cursor.fetchone()
