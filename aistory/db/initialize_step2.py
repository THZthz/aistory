...
from aistory.types.item import EItem, ItemClass, Qualities


def _update_items(cursor):
    # Load from TOML file.
    items_catalog = tomllib.load(open(PATH('database/catalog.items.toml'), 'rb'))['Catalog']['Items']
    ...
    def insert_catalog():
        stmt = '''INSERT INTO "ItemTypesTable" (type_id, type_name, category_id, type_quality, type_description, type_attributes)                                 
                VALUES (%s, %s, %s, %s, %s, %s)'''
...
def _update_locations(cursor):
    # Load from TOML file.
    locations_catalog = tomllib.load(open(PATH('database/catalog.locations.toml'), 'rb'))['Catalog']['Locations']
    ...
    cursor.executemany('''
        INSERT INTO "LocationsTable" (location_id, location_name, location_type, location_description, location_path) 
...
def _insert_characters(cursor):
    def callback(file_path):
        ...
        ids = execute_values(
            cursor,
            f'''INSERT INTO "ItemsTable" (item_type) VALUES {'%s'} RETURNING item_id''',
